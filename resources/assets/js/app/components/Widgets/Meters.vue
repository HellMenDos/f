				...
				<table class="table">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center">{{ METERS.tariff.electricity_day }}</td>
							<td class="text-center">{{ METERS.prev.electricity_day }}</td>
							<td class="text-center">{{ METERS.currency.electricity_day }}</td>
	...
	export default {
		...
		computed: {
			...
			electricityCost: function () {
				return parseFloat(this.METERS.currency.electricity_day - this.METERS.prev.electricity_day).toFixed(2);
			},
			...
			electricityTotal: function () {
				return parseFloat(this.METERS.tariff.electricity_day * this.electricityCost).toFixed(2);
			},
		...
		methods: {
			...
			onSubmitMeters() {
				...
				axios.post(route('api.meters_data.store'), {
