...
@section('after-scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#executor').select2({tags: [], theme: 'bootstrap'});
        });
    </script>
            ...
            <section class="block">
                ...
                <div class="block_body">
                    ...
                    <fieldset>
                        ...
                        <div class="form-group">
                            <label for="executor">Исполнитель <span class="required">*</span></label>
                            <select id="executor" name="executor_id" class="form-control">
                                <option value="0">-- Выбрать --</option>
                                @foreach ($executors as $executor)
                                    <option value="{{ $executor->id }}"
                                            @php selected($executor->id, $service->executor_id) @endphp
                                    >
                                        {{ $executor->title }}
                                    </option>
                                @endforeach
                            </select>
                            <span class="help-block">Выберите исполнителя из списка</span>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" rows="16" id="description" class="form-control wysiwyg"
                                      required="">{{ old('fulltext', html_decode($service->description)) }}</textarea>
                        </div>
                    ...
                    <fieldset>
                            ...
                            <label>
                                ...
                                @isset($service->published)
                                    <input type="checkbox" name="published"
                                           value="1"@php checked($service->published == '1') @endphp>
                                @else
                        ...
                        @isset($service->updated_at)
                            <div class="form-group">
                                ...
                                <div class="datetimepicker input-group date">
                                    <input data-date-format="YYYY-MM-DD HH:mm:ss"
                                           value="{{ old('updated_at', $service->updated_at) }}" type="text"
                                           class="form-control" name="updated_at">
                                    <span class="input-group-addon" style="cursor: pointer;"><span
                                            class="fa-calendar fa"></span></span>
                                </div>
                            </div>
                        ...
                        {{--<div class="form-group">--}}
                            ...
                            {{--<div class="datetimepicker input-group date">--}}
                                {{--<input data-date-format="YYYY-MM-DD HH:mm:ss"--}}
                                       {{--value="{{ old('execution_date', $service->execution_date) }}" type="text"--}}
                                       {{--class="form-control" name="execution_date">--}}
                                {{--<span class="input-group-addon" style="cursor: pointer;"><span--}}
                                        {{--class="fa-calendar fa"></span></span>--}}
                            {{--</div>--}}
                        {{--</div>--}}
                        ...
                        <div class="form-group">
                            <label for="price">Цена, руб. <span class="required">*</span></label>
                            <input type="text" name="price"
                                   value="{{ old('price', $service->price) }}" id="price" class="form-control"
                                   required="">
                        </div>

                        <div class="form-group">
                            <label for="unit_id">Единица измерения <span class="required">*</span></label>
                            <select name="unit_id" id="unit_id" required="" class="form-control">
                                <option value="0">-- Выбрать --</option>
                                @foreach ($units as $unit)
                                    <option value="{{ $unit->id }}" @php selected($unit->id, $service->unit_id) @endphp>{{ $unit->title }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select id="group_id" name="group_id" tabindex="-1" class="form-control" required="">
                                ...
                                @foreach($servicesGroups as $group)
                                    ...
                                    @isset($service->group_id)
                                        <option
                                            value="{{ $group->id }}"@php selected($group->id == $service->group_id) @endphp>{{ $group->title }}</option>
                                    @elseif($selectGroup)
                                        <option
