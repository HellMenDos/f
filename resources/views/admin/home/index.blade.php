...
@section('after-scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            const barWidth=($('#bar').width()/3)*(0.4);
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($area->status == 'no-data-water')
                                        <a href="{{ route('admin.areas.show', $area->id) }}?tab=tab-meters-data" class="js-modal badge badge-default" data-remote="{{ route('admin.areas.ajax_set_meters', $area->id) }}">Водоснабжение</a>
                                    @elseif ($area->status == 'no-data-electricity')
                                        <a href="{{ route('admin.areas.show', $area->id) }}?tab=tab-meters-data" class="js-modal badge badge-warning" data-remote="{{ route('admin.areas.ajax_set_meters', $area->id) }}">Электроэнергия</a>
                                    @elseif ($area->status == 'no-data')
                                        <a href="{{ route('admin.areas.show', $area->id) }}?tab=tab-meters-data" class="badge badge-danger">Показания не указаны!</a>
                                    @endif
                                </td>
     ...
     <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
        <div class="block">
            <div class="block_header">
                ...
                <div class="action">
                    <a href="{{ route('admin.services.request.index') }}" title="Все заявки" class="btn btn-default btn-xs"><i class="fa fa-list"></i></a>
                </div>
            </div>
            <div class="block_body">
                <table class="table table-striped table-condensed">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="requests__descrip" title="">
                                @if ($service->description)
                                    {{ Str::limit(strip_tags(html_decode($service->description)), 35) }}
                                @else
                            ...
                            <td class="col-action">
                                <div class="btn-group"><a class="btn btn-default btn-xs" href="{{ route('admin.services.request.show', $service->id) }}"><i class="fa fa-search"></i></a></div>
                            </td>
                        </tr>
                        @empty
                            <tr>
...
<div class="row">
    ...
    <div class="metersData">
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">Внести показания</button>
    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title" id="exampleModalLabel">Показания счетчиков э/э</h2>
                </div>
                <div class="modal-body">
                    <table class="table table-striped table-condensed">
                        ...
                        <tr>
                            ...
                            {{--                        <th>Адрес</th>--}}
                            {{--                        <th>Месяц подачи</th>--}}
                            {{--                        <th>Текущий тариф - день</th>--}}
                            {{--                        <th>Текущий тариф - ночь</th>--}}
                            <th>Предыдущие показания - день</th>
                            <th>Предыдущие показания - ночь</th>
                            {{--                        <th>Ошибки</th>--}}
                            <th>Ввести показания - день</th>
                            <th>Ввести показания - ночь</th>
                            <th>Текущие показания - день</th>
                            <th>Текущие показания - ночь</th>
                            {{--                        <th>Потребляемые квт/ч - день</th>--}}
                            {{--                        <th>Потребляемые квт/ч - ночь</th>--}}
                            {{--                        <th>Сумма - день</th>--}}
                            {{--                        <th>Сумма - ночь</th>--}}
                            {{--                        <th>Итого</th>--}}
                        </tr>
                        ...
                        <tbody>
                        @foreach($meters_data as $md)
                            <form id="contactform" method="get" class="validateform">
                                {{ csrf_field() }}
                                <tr>
                                    {{--                                <td><input name="full_name" value="{{$area->user_area->user->name}}" disabled /></td>--}}
                                    {{--                                <td><input name="address" value="{{$area->address}}" disabled /></td>--}}
                                    <input type="hidden" name="area_id" value="{{$md->area_id}}" />
                                    {{--                                <td><input type="hidden" name="month" value="{{$md->created_at}}" /></td>--}}
                                    {{--                                <td><input name="tariff_day" value="{{$tariff->electricity}}" disabled /></td>--}}
                                    {{--                                <td><input name="tariff_night" value="{{$tariff->electricity_night}}" disabled /></td>--}}
                                    <td><input name="last_electricity_day" value="{{$md->last_electricity_day}}" disabled /></td>
                                    <td><input name="last_electricity_night" value="{{$md->last_electricity_night}}" disabled /></td>
                                    {{--                            <td>ошибки</td>--}}
                                    <td><input type="text" name="insertDataDay" /></td>
                                    <td><input type="text" name="insertDataNight" /></td>
                                    <td><input name="electricity_day" value="{{$md->electricity_day}}" readonly /></td>
                                    <td><input name="electricity_night" value="{{$md->electricity_night}}" readonly /></td>
                                    {{--                                <td><input name="consumed_kwh_day" value="{{$kwh_day=$md->electricity_day-$md->last_electricity_day}}" disabled /></td>--}}
                                    {{--                                <td><input name="consumed_kwh_night" value="{{$kwh_night=$md->electricity_night-$md->last_electricity_night}}" disabled /></td>--}}
                                    {{--                                <td>{{$sum1=$md->consumed_kwh_day*$tariff->electricity}}</td>--}}
                                    {{--                                <td>{{$sum2=$md->consumed_kwh_night*$tariff->electricity_night}}</td>--}}
                                    {{--                                <td>{{$sum1+$sum2}}</td>--}}
                                    <td><button class="btn btn-default btn-xs">
                                            <span class="hidden-xs">Сохранить</span><i class="fa fa-plus"></i>
                                        </button></td>
                                    <td><a href="#" class="btn btn-default btn-xs" type="submit">
                                            <span class="hidden-xs">Изменить</span> <i class="fa fa-bars"></i>
                                        </a></td>
                                </tr>
                            </form>
