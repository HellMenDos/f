    ...
    public function getMeters(Request $request)
    {
        ...
        foreach ($metersDataAll as $k => $meter) {
            if ($k > 0) {
                ...
                $metersDataAll[$k]->electricityMeters = $metersDataAll[$k]->electricity_day - $metersDataAll[$k - 1]->electricity_day;
                ...
                $counters = json_decode($area->сounters);

                $metersDataAll[$k]->waterMeters = $counters->water_first_meter;
                $metersDataAll[$k]->electricityMeters = $counters->electr_first_meter;
                $metersDataAll[$k]->electricityNightMeters = $counters->electr_night_first_meter;
            }
    ...
    public function getMetersHistory(Request $request)
    {
        ...
        foreach ($metersData as $k => $meter) {
            ...
            if ($k > 0) {
                ...
                $electricityMeters = $metersData[$k]->electricity_day - $metersData[$k - 1]->electricity_day;
                ...
                $waterMeters = $counters->water_first_meter;
                $electricityMeters = $counters->electr_first_meter;
                $electricityNightMeters = $counters->electr_night_first_meter;
            }

            $metersArr['water'][] = ($waterMeters > 0) ? $waterMeters : 0;
            $metersArr['electricity_day'][] = ($electricityMeters > 0) ? $electricityMeters : 0;
